...
const RecursoBlock = require('../errors/RecursoBlock');
...
module.exports = (app) => {
  ...
  router.param('id',(req, res ,next) => {
    app.services.account.find({ id: req.params.id })
      .then((acc) => {
        if (acc.user_id != req.user.id) throw new RecursoBlock();
        else next();
      }).catch(err => next(err));
  });
  //verificação de id das rotas
  ...
  }); /* Tratamento de erro via middleware do app */
  ...
  router.put('/:id',(req, res, next) => {
    app.services.account.update( req.params.id, req.body )
      .then(result => res.status(200).json(result[0]))//retorna o primeiro valor
      .catch(err => next(err));
  });
